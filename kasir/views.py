        ...
        if form.is_valid():
            ...
            for key, value in request.POST.items():
                if(Menu.objects.filter(nama=str(key)).exists()):
                    ...
                    DetailTransaksi.objects.create(transaksi=transaksi, menu=Menu.objects.filter(nama=str(key)).first(), jumlah=value, harga=int(Menu.objects.filter(nama=str(key)).first().harga))
            transaksi.save()
